window.onload = function(){
	...
	var oBtn = document.getElementsByClassName("jdzy_check")[0];
	oBtn.onclick = gwc;
	function gwc(){
		 var hasChecked = oBtn.getAttribute('checked');
		 if(hasChecked !== null){   
			for(var j = 0 ; j < checkBtn.length; j++){ 
               checkBtn[j].removeAttribute('checked');
            }
        }else{
        	for(var j = 0 ; j < checkBtn.length; j++){ 
        	checkBtn[j].setAttribute('checked',' ');
        	}
        }
	}
//	
//	var oBtn1 = document.getElementById("jdzy");
////	oBtn1.onclick=gwc;
